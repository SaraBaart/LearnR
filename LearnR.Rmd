...
**Note ** Create a new data set with a new name. Otherwise you lose your data.

Remove observations with missings in "mpg" and "cyl"
```{r echo = T, eval=T}
...
In the data "am" is a grouping variable (0/1). 
```{r echo = T, eval=T}
table(data$am)
```
...
##### Mann-Whitney U test
When the continuous variable is not normally distributed, we use the Mann-Whitney U test (or Wilcoxon Rank Sum test). The variable "mpg" is not normally distributed
